...
def load_data(table_name):
    response = supabase.table(table_name).select('date_string').execute()
...
def Other_Page():
    ...
    weekly_count = 0
    for day in last_7_days:
        if day in data_journal['completed_days']:
            weekly_count += 1
    ...
    #The following is button and calendar logic. Identical to the diet tracker
     # FIXED: Restructured the button logic scope
    if yesterday in data_journal['completed_days']:
        st.success('Good job!')
        if st.button('Unmark Yesterday'):
            save_data('journal_data',yesterday, action='remove')
            st.rerun()
    else:
        if st.button('Complete Yesterday', use_container_width=True):
            save_data('journal_data', yesterday, action='add')
            st.rerun()
    ...
    for i in range(30):
        ...
        if current_date == today:
            emoji = "⬜"
        elif current_date in data_journal['completed_days']:
            emoji = "✅"
        else: 
            emoji = "🟥"
    ...
    streak = 0
    check_date = datetime.now() - timedelta(days=1)
    while check_date.strftime("%Y-%m-%d") in data_journal['completed_days']:
        streak += 1
        check_date -= timedelta(days=1)
    ...
    # Goal logic
    total_completed = len(data_journal['completed_days'])
    weekly_goal = 7
    monthly_goal = 31
    progress_monthly = min(total_completed/monthly_goal, 1.0)
